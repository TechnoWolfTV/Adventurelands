...
local S = core.get_translator("unifieddyes")
...
function unifieddyes.on_airbrush(itemstack, player, pointed_thing)
	...
	if not painting_with then
		core.chat_send_player(player_name, "*** You need to set a color first.")
		core.chat_send_player(player_name, "*** Right-click any random node to open the color selector,")
		core.chat_send_player(player_name, "*** or shift+right-click a colorized node to use its color.")
		core.chat_send_player(player_name, '*** Be sure to click "Accept", or the color you select will be ignored.')
		return
	end
	...
	if not pos then
		...
		if look_angle > -1.55 then
			core.chat_send_player(player_name, "*** No node selected")
		else
	...
	if core.is_protected(pos, player_name) then
		core.chat_send_player(player_name, "*** Sorry, someone else owns that node.")
		return
	end

	if not (def.groups and def.groups.ud_param2_colorable and def.groups.ud_param2_colorable > 0) then
		core.chat_send_player(player_name, "*** That node can't be colored.")
		return
	end
	...
	if not def or not def.palette then
		core.chat_send_player(player_name, "*** That node can't be colored -- it's either undefined or has no palette.")
		return
	...
	then
		...
		core.chat_send_player(player_name, "*** That node can't be colored -- it has an invalid color mode.")
		return
	end

	local idx, hue = unifieddyes.getpaletteidx(painting_with, palette)
	local inv = player:get_inventory()
	if (not core.is_creative_enabled(player_name)) and not inv:contains_item("main", painting_with) then
		local suff = ""
		if not idx then
	...
	if not idx then
		core.chat_send_player(player_name, "*** " .. string.sub(painting_with, 5) .. " can't be applied to that node.")
		return
	end
	...
	if palette == "split" then
		...
		if not core.registered_items[name] then
			core.chat_send_player(
				player_name,
				"*** " .. string.sub(painting_with, 5) .. " can't be applied to that node."
			)
			return
		end
	...
	core.swap_node(pos, { name = name, param2 = fdir + idx })
...
function unifieddyes.make_readable_color(color)
	...
	s = string.gsub(s, "(%l)(%w*)", function(a, b)
...
function unifieddyes.make_colored_square(
	hexcolor,
	colorname,
	showall,
	creative,
	...
	local overlay = ""
	local colorize = core.formspec_escape("^[colorize:#" .. hexcolor .. ":255")
	...
	if
		not showall and not unifieddyes.palette_has_color[nodepalette .. "_" .. colorname]
		or (explist and not explist[colorname])
	then
		if overlay == "" then
...
function unifieddyes.show_airbrush_form(player)
	...
	if last_right_click then
		...
		elseif last_right_click.def.palette then
			...
			elseif last_right_click.def.palette == "unifieddyes_palette_extended.png" then
				t[#t + 1] = "label[0.5,8.25;"
					.. S("(Right-clicked a node that supports all 256 colors, showing them all)")
					.. "]"
				showall = true
			elseif
				last_right_click.def.palette ~= "unifieddyes_palette_extended.png"
				and last_right_click.def.palette ~= "unifieddyes_palette_colorwallmounted.png"
				and string.find(last_right_click.def.palette, "unifieddyes_palette_")
			then
				nodepalette = "split"
			end
		end
	...
	elseif
		not last_right_click.def.groups
		or not last_right_click.def.groups.ud_param2_colorable
		or not last_right_click.def.palette
		or not string.find(last_right_click.def.palette, "unifieddyes_palette_")
	then
		t[#t + 1] = "label[0.5,8.25;"
			.. S("(Right-clicked a node not supported by the Airbrush, showing all colors)")
			.. "]"
	end
	...
	for v = 0, 6 do
		...
		for hi, h in ipairs(unifieddyes.HUES_EXTENDED) do
			...
			local f
			f, selindic = unifieddyes.make_colored_square(
				hexcolor,
				val .. hue .. sat,
				showall,
				creative,
	...
	t[#t + 1] = "button_exit[10.5,8;2,1;cancel;"
		.. S("Cancel")
		.. "]button_exit[12.5,8;2,1;accept;"
		.. S("Accept")
		.. "]"
	...
	core.show_formspec(player_name, "unifieddyes:dye_select_form", table.concat(t))
end
	...
	if formname == "unifieddyes:dye_select_form" then
		...
		elseif fields.quit then
			if fields.accept then
				...
				if not dye then
					core.chat_send_player(player_name, '*** Clicked "Accept", but no color was selected!')
					return
				elseif not showall and not unifieddyes.palette_has_color[nodepalette .. "_" .. string.sub(dye, 5)] then
					core.chat_send_player(
						player_name,
						'*** Clicked "Accept", but the selected color can\'t be used on the'
					)
					core.chat_send_player(
						player_name,
						'*** node that was right-clicked (and "Show All" wasn\'t in effect).'
					)
					if unifieddyes.player_current_dye[player_name] then
						core.chat_send_player(
							player_name,
							"*** Ignoring it and sticking with "
								.. string.sub(unifieddyes.player_current_dye[player_name], 5)
								.. "."
						)
					else
						core.chat_send_player(player_name, "*** Ignoring it.")
					end
					return
				else
					unifieddyes.player_current_dye[player_name] = dye
					unifieddyes.player_selected_dye[player_name] = nil
					core.chat_send_player(player_name, "*** Selected " .. string.sub(dye, 5) .. " for the airbrush.")
					return
				end
			...
			if
				(showall or unifieddyes.palette_has_color[nodepalette .. "_" .. color])
				and (creative or inv:contains_item("main", dye))
			then
...
core.register_tool("unifieddyes:airbrush", {
	...
	on_place = function(itemstack, placer, pointed_thing)
		...
		elseif keys.aux1 then
			...
			if newcolor and string.find(def.paramtype2, "color") then
				core.chat_send_player(
					player_name,
					"*** Switching to " .. newcolor .. " for the airbrush, to match that node."
				)
				unifieddyes.player_current_dye[player_name] = "dye:" .. newcolor
			else
				core.chat_send_player(player_name, "*** That node is uncolored.")
			end
		end
	end,
})
