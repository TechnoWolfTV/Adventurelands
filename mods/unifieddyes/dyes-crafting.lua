...
for _, h in ipairs(unifieddyes.HUES_EXTENDED) do
	...
	for v = 0, 6 do
		...
		if core.registered_items["dye:" .. val .. hue] then
			core.override_item("dye:" .. val .. hue, {
				inventory_image = "unifieddyes_dye.png^[colorize:#" .. color .. ":200",
			})
		else
			if (val .. hue) ~= "medium_orange" and (val .. hue) ~= "light_red" then
				core.register_craftitem(":dye:" .. val .. hue, {
					description = S(desc),
					inventory_image = "unifieddyes_dye.png^[colorize:#" .. color .. ":200",
		...
		core.register_alias("unifieddyes:" .. val .. hue, "dye:" .. val .. hue)
		...
		if v > 3 then -- also register the low-sat version
			...
			core.register_craftitem(":dye:" .. val .. hue .. "_s50", {
				description = S(desc .. " (low saturation)"),
				inventory_image = "unifieddyes_dye.png^[colorize:#" .. ccolor .. ":200",
				groups = { dye = 1, not_in_creative_inventory = 1 },
			})
			core.register_alias("unifieddyes:" .. val .. hue .. "_s50", "dye:" .. val .. hue .. "_s50")
		end
	end
end
...
for y = 1, 14 do -- colors 0 and 15 are black and white, default dyes
	if y ~= 4 and y ~= 8 and y ~= 11 then -- don't register the three greys, they're done separately.
		...
		core.register_alias("unifieddyes:" .. name, "dye:" .. name)
	end
end

if core.get_modpath("dye") then
	core.override_item("dye:grey", {
		inventory_image = "unifieddyes_dye.png^[colorize:#888888:200",
	})

	core.override_item("dye:dark_grey", {
		inventory_image = "unifieddyes_dye.png^[colorize:#444444:200",
	})

	core.register_craftitem(":dye:light_grey", {
