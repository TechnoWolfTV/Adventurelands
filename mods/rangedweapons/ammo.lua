...
if minetest.get_node(moveresult.collisions[1].node_pos).name == "rangedweapons:barrel"
		and rangedweapons_item_active("rangedweapons:barrel") then
...
if moveresult.collisions[1].object:is_player() then
   ...
   local knockback = damage.knockback or 0
...
-- owner can be nil if the shooter disconnected while the bullet was in
-- flight, and the hud id must be that player's own.
if owner then
	local hud_id = rangedweapons_hud.hit[owner:get_player_name()]
	if hud_id then
		owner:hud_change(hud_id, "text", hit_texture)
	end
end
...
-- Optional override for ammo stack sizes.
--
-- Upstream scales stack_max inversely with ammo power (9mm 500 ... rocket 15),
-- which is a deliberate balance curve, not an oversight. Set
-- rangedweapons_ammo_stack_max to a positive number to flatten it instead.
-- 0 keeps the per-ammo defaults.
local ammo_stack_override = tonumber(core.settings:get("rangedweapons_ammo_stack_max")) or 0

if ammo_stack_override > 0 then
	core.register_on_mods_loaded(function()
		local n = 0
		for name, def in pairs(core.registered_items) do
			if name:sub(1, 14) == "rangedweapons:" and def.RW_ammo_capabilities then
				core.override_item(name, {stack_max = ammo_stack_override})
				n = n + 1
			end
		end
		core.log("action", ("[rangedweapons] ammo stack_max forced to %d for %d items")
			:format(ammo_stack_override, n))
	end)
end
