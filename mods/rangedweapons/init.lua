...
-- HUD element ids are per-player. Storing them in bare globals meant every
-- player shared the last-joined player's ids, so hit markers and the scope
-- overlay were applied to the wrong player on any multiplayer server.
rangedweapons_hud = {hit = {}, scope = {}}

-- Neutral hooks. Stock rangedweapons allows everything; the bundled admin
-- panel overrides these so a disabled item stops working rather than merely
-- becoming uncraftable. Kept here so the rest of the mod has no dependency on
-- the panel being present.

--- May this ammunition be loaded into a gun?
function rangedweapons_ammo_allowed(_, _)
	return true
end

--- Is this item still functional? Used for world nodes that keep acting after
--- they are placed, such as the explosive barrel.
function rangedweapons_item_active(_)
	return true
end
...
core.register_node("rangedweapons:antigun_block", {
...
function make_sparks(pos)
	core.sound_play("rengedweapons_ricochet", {pos = pos, gain = 0.75}, true)
	for i = 1, 9 do
		core.add_particle({
			pos = pos,
...
local max_gun_efficiency = tonumber(core.settings:get("rangedweapons_max_gun_efficiency")) or 300

function rangedweapons_gain_skill(player, skill, chance)
...
--- Empty a gun back into the player's inventory.
---
--- Deliberately works even when the gun or its ammunition has been disabled by
--- an admin: retrieving your own property from a container is not "using" it,
--- and without this exception disabled ammunition would be stuck in the gun.
function rangedweapons_unload_gun(itemstack, player)
	local gunMeta = itemstack:get_meta()
	local ammoCount = gunMeta:get_int("RW_bullets")
	local ammoName = gunMeta:get_string("RW_ammo_name")

	if ammoCount <= 0 or ammoName == "" then
		core.sound_play("rangedweapons_empty", {pos = player:get_pos()}, true)
		return false
	end

	local inv = player:get_inventory()
	local leftover = inv:add_item("main", ammoName .. " " .. ammoCount)
	if leftover and not leftover:is_empty() then
		core.add_item(player:get_pos(), leftover)
	end

	gunMeta:set_int("RW_bullets", 0)
	gunMeta:set_string("RW_ammo_name", "")
	gunMeta:set_string("count_meta", "0")

	local GunCaps = itemstack:get_definition().RW_gun_capabilities
	local snd = (GunCaps and GunCaps.gun_unload_sound) or "rangedweapons_handgun_mag_out"
	core.sound_play(snd, {pos = player:get_pos()}, true)

	player:get_meta():set_float("rw_cooldown", 0.5)

	core.chat_send_player(player:get_player_name(),
		core.colorize("#7ddc7d", ("Unloaded %d round(s)."):format(ammoCount)))

	return true
end

--- Is the player asking to unload rather than reload?
local function wants_unload(player)
	return player and player.get_player_control and player:get_player_control().sneak
end

function rangedweapons_reload_gun(itemstack, player)
	if wants_unload(player) then
		rangedweapons_unload_gun(itemstack, player)
		return
	end
	...
	core.sound_play(gun_unload_sound, {pos = player:get_pos()}, true)
	...
	if GunCaps.suitable_ammo ~= nil then
		...
		for i = 1, inv:get_size("main") do
			for _, ammo in pairs(GunCaps.suitable_ammo) do
				if inv:get_stack("main", i):get_name() == ammo[1]
						and rangedweapons_ammo_allowed(ammo[1], player) then
	...
	if player_has_ammo == 1 then
		...
		if core.settings:get_bool("rangedweapons_infinite_ammo", false) then
			gunMeta:set_int("RW_bullets", clipSize)
		else
			-- add_item returns whatever did not fit. Discarding it silently
			-- deleted the unloaded ammo whenever the player's inventory was
			-- full, so drop the remainder at their feet instead.
			local leftover = inv:add_item("main", ammoName .. " " .. ammoCount)
			if leftover and not leftover:is_empty() then
				core.add_item(player:get_pos(), leftover)
			end

			local item = reload_ammo:get_name() .. " " .. clipSize
...
function rangedweapons_single_load_gun(itemstack, player)
	if wants_unload(player) then
		rangedweapons_unload_gun(itemstack, player)
		return
	end
	...
	if player_has_ammo == 1 then
		...
		if inv:contains_item("main", reload_ammo:get_name()) and gunMeta:get_int("RW_bullets") < clipSize then
			if not core.settings:get_bool("rangedweapons_infinite_ammo", false) then
...
function rangedweapons_yeet(itemstack, player)
	local playerPos = player:get_pos()
	local playerName = player:get_player_name()
	if core.find_node_near(playerPos, 10, "rangedweapons:antigun_block") then
		core.chat_send_player(playerName, core.colorize("#ff0000", "Throwable weapons are prohibited in this area!"))
	else
		...
		if playerMeta:get_float("rw_cooldown") <= 0 then
			...
			-- TODO: this might be the longest set of parameters I've ever seen
			rangedweapons_launch_projectile(player, throw_projectiles, throw_damage, throw_ent,
				throw_visual, throw_texture, throw_sound, throw_velocity, throw_accuracy,
				skill_value, OnCollision, throw_crit, throw_critEffc, throw_mobPen,
				throw_nodePen, 0, "", "", "", throw_dps, throw_gravity, throw_door_breaking,
				throw_glass_breaking, throw_particles, throw_sparks, throw_bomb_ignite,
				throw_size, 0, itemstack:get_wear(), throw_glow
			)
...
function rangedweapons_shoot_gun(itemstack, player)
	if core.find_node_near(player:get_pos(), 10, "rangedweapons:antigun_block") then
		core.sound_play("rangedweapons_empty", {pos = player:get_pos()}, true)
		core.chat_send_player(player:get_player_name(), core.colorize("#ff0000", "Guns are prohibited in this area!"))
	else
		...
		if gunMeta:get_int("RW_bullets") > 0 and playerMeta:get_float("rw_cooldown") <= 0 then
			...
			rangedweapons_launch_projectile(player, combined_projNum, combined_dmg, bullet_ent,
				bullet_visual, bullet_texture, gun_sound, combined_velocity, gun_accuracy,
				skill_value, OnCollision, combined_crit, combined_critEffc, combined_mobPen,
				combined_nodePen, gun_shell, bullet_shell_ent, bullet_shell_texture,
				bullet_shell_visual, combined_dps,combined_gravity, gun_door_breaking,
				bullet_glass_breaking, bullet_particles, bullet_sparks, bullet_bomb_ignite,
				bullet_size, gun_smokeSize, 0, bullet_glow
			)

			if core.settings:get_bool("rangedweapons_gun_wear", true)
					and gun_durability > 0 then
...
function rangedweapons_shoot_powergun(itemstack, player)
	if core.find_node_near(player:get_pos(), 10, "rangedweapons:antigun_block") then
		core.sound_play("rangedweapons_empty", {pos = player:get_pos()}, true)
		core.chat_send_player(player:get_player_name(), core.colorize("#ff0000", "Guns are prohibited in this area!"))
	else
		...
		if inv:contains_item("main", "rangedweapons:power_particle " .. PowerCaps.power_consumption) and playerMeta:get_float("rw_cooldown") <= 0 then
			...
			rangedweapons_launch_projectile(player, power_projectiles, power_damage, power_ent,
				power_visual, power_texture, power_sound, power_velocity, power_accuracy,
				skill_value, OnCollision, power_crit, power_critEffc, power_mobPen,
				power_nodePen, 0, "", "", "", power_dps, power_gravity, power_door_breaking,
				power_glass_breaking, power_particles, power_sparks, power_bomb_ignite,
				power_size, 0, 0, power_glow
			)

			if core.settings:get_bool("rangedweapons_gun_wear", true)
					and power_durability > 0 then
...
function rangedweapons_launch_projectile(player, projNum, projDmg, projEnt,
		visualType, texture, shoot_sound, combined_velocity, accuracy,
		skill_value, ColResult, projCrit, projCritEffc, mobPen, nodePen,
		has_shell, shellEnt, shellTexture, shellVisual, dps, gravity,
		door_break, glass_break, bullet_particles, sparks, ignite, size,
		smokeSize, proj_wear, proj_glow)
	...
	if pos and dir and yaw then
		...
		for i = 1, projectiles do
			local obj = core.add_entity(pos, projEnt)
...
function eject_shell(itemstack, player, rld_item, rld_time, rldsound, shell)
	-- Loaded shotguns and bolt-action rifles put shell ejection on right-click,
	-- so the unload gesture has to be handled here too.
	if wants_unload(player) then
		rangedweapons_unload_gun(itemstack, player)
		return
	end
	...
	if pos and dir and yaw then
		...
		-- Was `bulletStack ~= ""`, comparing an ItemStack to a string, which is
		-- always true. A gun with no ammo name in meta then indexed a nil
		-- AmmoCaps and crashed.
		local AmmoCaps = not bulletStack:is_empty()
			and bulletStack:get_definition().RW_ammo_capabilities or nil

		if AmmoCaps then
			local bullet_shell_visual = AmmoCaps.shell_visual or "wielditem"
			local bullet_shell_texture = AmmoCaps.shell_texture or "rangedweapons:shelldrop"
...
if core.settings:get_bool("rangedweapons_shurikens", true) then
	dofile(modpath .. "/shurikens.lua")
end

if core.settings:get_bool("rangedweapons_handguns", true) then
...
if core.settings:get_bool("rangedweapons_forceguns", true) then
	dofile(modpath .. "/forcegun.lua")
end

if core.settings:get_bool("rangedweapons_javelins", true) then
	dofile(modpath .. "/javelin.lua")
end

if core.settings:get_bool("rangedweapons_power_weapons", true) then
...
if core.settings:get_bool("rangedweapons_machine_pistols", true) then
...
if core.settings:get_bool("rangedweapons_shotguns", true) then
	dofile(modpath .. "/remington.lua")
	dofile(modpath .. "/spas12.lua")
	dofile(modpath .. "/benelli.lua")
end

if core.settings:get_bool("rangedweapons_auto_shotguns", true) then
	dofile(modpath .. "/jackhammer.lua")
	dofile(modpath .. "/aa12.lua")
end

if core.settings:get_bool("rangedweapons_smgs", true) then
...
if core.settings:get_bool("rangedweapons_rifles", true) then
	dofile(modpath .. "/awp.lua")
	dofile(modpath .. "/svd.lua")
	dofile(modpath .. "/m200.lua")
end

if core.settings:get_bool("rangedweapons_heavy_machineguns", true) then
	dofile(modpath .. "/m60.lua")
	dofile(modpath .. "/rpk.lua")
	dofile(modpath .. "/minigun.lua")
end

if core.settings:get_bool("rangedweapons_revolvers", true) then
	dofile(modpath .. "/python.lua")
	dofile(modpath .. "/taurus.lua")
end

if core.settings:get_bool("rangedweapons_assault_rifles", true) then
...
if core.settings:get_bool("rangedweapons_explosives", true) then
...
if core.settings:get_bool("rangedweapons_glass_breaking", true) then
...
rangedweapons_empty_shell.on_step = function(self, dtime, _)
...
core.register_on_joinplayer(function(player)
	...
	rangedweapons_hud.hit[pname] = player:hud_add({
		type = "image",
	...
	rangedweapons_hud.scope[pname] = player:hud_add({
		type = "image",
		position = { x = 0.5, y = 0.5 },
		scale = { x = -100, y = -100 },
		text = "rangedweapons_empty_icon.png",
...
core.register_on_leaveplayer(function(player)
	local pname = player:get_player_name()
	rangedweapons_hud.hit[pname] = nil
	rangedweapons_hud.scope[pname] = nil
end)
...
core.register_globalstep(function(dtime)
	...
	if timer >= 1.0 then
		for _, player in pairs(core.get_connected_players()) do
			local hud_id = rangedweapons_hud.hit[player:get_player_name()]
			if hud_id then
				player:hud_change(hud_id, "text", "rangedweapons_empty_icon.png")
			end
			timer = 0
		end
	end
...
----
---- admin panel (see admin/ and BUNDLING.md)
----
