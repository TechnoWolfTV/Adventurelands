...
local on_hit_remove = function(self)
   minetest.sound_play(
      bows_epic.registered_arrows[self.name].on_hit_sound, {
         pos = self.object:get_pos(),
         gain = 1.0,
         max_hear_distance = 7
      })
   -- chance of dropping arrow
   local chance = bows_epic.registered_arrows[self.name].drop_chance
...
local on_hit_object = function(self, target, hp, user, lastpos)
   ...
   if bows_epic.registered_arrows[self.name].on_hit_object then
      bows_epic.registered_arrows[self.name].on_hit_object(
...
minetest.register_entity("bows_epic:arrow",{
   hp_max = 10,
   visual = "wielditem",
   ...
   on_activate = function(self, staticdata)
      ...
      if bows_epic.tmp and bows_epic.tmp.arrow ~= nil then
         self.arrow = bows_epic.tmp.arrow
         self.user = bows_epic.tmp.user
         self.name = bows_epic.tmp.name
         self.dmg = bows_epic.registered_arrows[self.name].damage
         self.text = bows_epic.registered_arrows[self.name].disp_entity
         bows_epic.tmp = nil
   ...
   on_step = function(self, dtime, ...)
      ...
      if vel.x == 0 or vel.y == 0 or vel.z == 0 then
         if bows_epic.registered_arrows[self.name].on_hit_node then
            ...
            bows_epic.registered_arrows[self.name].on_hit_node(
